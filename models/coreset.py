...
class Base:
    def __init__(self, data, args, device='cuda', **kwargs):
        ...
        self.device = device
        if args.induct==True:
            n = int(data.x.shape[0] * args.reduction_rate)
            d = data.x.shape[1]
        else :
            n = int(data.feat_full.shape[0] * args.reduction_rate)
            d = data.feat_full.shape[1]
    ...
    def generate_labels_syn(self, data):
        ...
        if self.args.induct==True:
            #counter = Counter(data.labels_train.cpu().numpy())
            counter = Counter(data.labels_train.cpu().numpy())
        else :
            counter = Counter(data.labels_full.cpu().numpy())
        num_class_dict = {}
        if self.args.induct==True:
            n = len(data.labels_train)
        else :
            n = len(data.labels_full)
...
class KCenter(Base):
    ...
    def select(self, embeds,):
        ...
        if self.args.induct:
            idx_train =torch.range(0,len(self.data.labels_train)-1) #self.data.train_mask #
        else:
            idx_train = self.data.idx_train_full
        labels_train = self.data.labels_train
        idx_selected = []
       
        for class_id, cnt in num_class_dict.items():
            idx = idx_train[labels_train.cpu()==class_id].long()
            feature = embeds[idx]
