    ...
    **kwargs : keyword arguments
    ...
    def train_cross(self, data, grid_search=True, reduced=True):
        ...
        if grid_search:
            ...
            eval_model_list = ['GCN', 'SGC', 'APPNP', 'Cheby', 'GraphSage', 'GAT','SGFormer']
            # eval_model_list = ['SGFormer']
    ...
    def test_LP(self, data, model_type, verbose=True, reduced=True, mode='eval', MIA=False):
        """
        Tests a model and returns accuracy and loss for edge prediction.
        ...
        mia_acc : float (optional)
            MIA accuracy for edge privacy.
        """
        args = self.args
        ...
        # Extract edge indices from adjacency matrices
        adj_train = data.adj_train
        adj_test = data.adj_test

        edge_index_train = torch.tensor([adj_train.row, adj_train.col], dtype=torch.long).to(self.device)
        edge_index_test = torch.tensor([adj_test.row, adj_test.col], dtype=torch.long).to(self.device)

        # Predict edge probabilities
        z = model.encode(data.feat_full, data.adj_full)
        edge_prob_train = model.decode(z, edge_index_train).sigmoid().view(-1)
        edge_prob_test = model.decode(z, edge_index_test).sigmoid().view(-1)

        # Compute edge prediction accuracy
        pred_test = (edge_prob_test > 0.5).float()
        edge_label_test = torch.ones_like(edge_prob_test)  # Assuming all edges in adj_test are positive
        acc_test = (pred_test == edge_label_test).float().mean().item()
        ...
        if MIA:
            # Perform edge-level MIA
            edge_label_train = torch.ones_like(edge_prob_train)  # Assuming all edges in adj_train are positive
            mia_acc = inference_via_confidence(
                edge_prob_train.cpu().numpy(),  # Confidence scores for training edges
                edge_prob_test.cpu().numpy(),   # Confidence scores for test edges
                edge_label_train.cpu().numpy(), # Labels for training edges
                edge_label_test.cpu().numpy()   # Labels for test edges
            )
    ...
    def MIA_evaluate_LP(self, data, model_type, verbose=True, reduced=True, mode='eval'):
        """
        Evaluates a model over multiple runs and returns mean and standard deviation of accuracy.
        ...
        for i in run_evaluation:
            seed_everything(args.seed + i)
            _, best_acc, mia_acc = self.test_LP(data, model_type=model_type, verbose=args.verbose, reduced=reduced,
                                            mode=mode, MIA=True)
            res.append(best_acc)
            mia_res.append(mia_acc)
            if verbose:
