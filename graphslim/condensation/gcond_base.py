    ...
    **kwargs : keyword arguments
    ...
    def train_class(self, model, adj, features, labels, labels_syn, args, soft=False):
        ...
        for c in range(data.nclass):
            ...
            batch_size, n_id, adjs = data.retrieve_class_sampler(c, adj, args, num=args.batch_num)
    ...
    def get_laplacian(self,adj):
        ...
        #adj_self_loop = adj + torch.eye(adj.shape, device=adj.device)
        adj_self_loop = adj + torch.eye(adj.shape[0], device=adj.device)
        deg = torch.sum(adj_self_loop, dim=1)
        deg_inv_sqrt = deg.pow(-0.5)
        deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0 # 处理度为0的孤立节点
    ...
    def intermediate_evaluation(self, best_val, loss_avg=None, save=True, save_valid_acc=False):
        ...
        args.logger.info('Val:  {:.4f} +/- {:.4f}'.format(100*res[0].mean(), 100*res[0].std()))
