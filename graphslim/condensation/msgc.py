...
class MSGC(GCondBase):
    ...
    def __init__(self, setting, data, args, **kwargs):
        ...
        self.num_class_dict = self.data.num_class_dict = {index: value for index, value in enumerate(n_each_y) if value != 0}
    ...
    def generate_labels_syn(self, data):
        ...
        rate = F.one_hot(labels_train, num_classes=data.nclass).sum(0).float()
        present_mask = rate > 0
        if present_mask.any():
            rate_present = rate[present_mask]
            rate_present = rate_present / rate_present.sum()
        else:
            present_mask = torch.ones_like(rate, dtype=torch.bool)
            rate_present = torch.full((data.nclass,), 1.0 / data.nclass, device=self.device)

        base_present = torch.ones_like(rate_present)
        available = n_syn - int(base_present.sum().item())
        if available < 0:
            base_present = torch.zeros_like(base_present)
            available = n_syn

        quota = rate_present * max(available, 0)
        n_each_present = torch.floor(quota).to(torch.long) + base_present.to(torch.long)

        left = int((n_syn - n_each_present.sum()).item()) if n_each_present.numel() > 0 else 0
        if n_each_present.numel() > 0 and left != 0:
            fractional = quota - torch.floor(quota)
            order = torch.argsort(fractional, descending=left > 0)
            limit = order.numel()
            step = 1 if left > 0 else -1
            for i in range(abs(left)):
                idx = order[i % limit]
                if left > 0:
                    n_each_present[idx] += 1
                elif n_each_present[idx] > 0:
                    n_each_present[idx] -= 1

        n_each_y = torch.zeros(data.nclass, device=self.device, dtype=torch.long)
        n_each_y[present_mask] = n_each_present.to(torch.long)
