...
def compute_roc_auc(labels: torch.Tensor, logits: torch.Tensor) -> float:
    """Compute ROC AUC from logits and integer labels."""
    if logits.dim() == 1:
    ...
    try:
        if num_classes == 2:
            # Use the positive class probability for binary problems
...
def evaluate(cfg, model, data, dataloader, device):
    model.eval()
    logits_list = []
    labels_list = []
    ...
    with torch.no_grad():
        if cfg.model_name == "MLP":
            # For MLP, we don't need neighbor sampling
            out = model(data.x[dataloader], None)
            logits_list.append(out.detach())
            labels_list.append(data.y[dataloader].detach())
        else:
            # For GNN models, use the dataloader
    ...
    return compute_roc_auc(labels_all, logits_all)

def create_pyg_data(edge_index, features, labels, train_mask, val_mask, test_mask, num_classes):
...
class MMGraphCompressor:
    ...
    def load_data_and_model(self,cfg ):
        ...
        if self.cfg.feat == "imagebind":
            dataset = torch.load(os.path.join(data_path, dataset_name + '_imagebind.pt'))
        elif self.cfg.feat == "t5vit":
            dataset = torch.load(os.path.join(data_path, dataset_name + '_t5vit.pt'))
        elif self.cfg.feat == "clip":
            dataset = torch.load(os.path.join(data_path, dataset_name + '_clip.pt'))
    ...
    def test_with_val(self, verbose=False):
        ...
        adj_full_norm = utils.normalize_adj_tensor(data.adj_full, sparse=True)
        ...
        for i in range(train_iters):
            ...
            with torch.no_grad():
                model.eval()
                output = model.forward(data.feat_full, adj_full_norm)
                logits_eval = output[data.test_mask]
                labels_eval = data.labels_test
                auc_val = compute_roc_auc(labels_eval, logits_eval)
                ...
                if auc_val > best_auc_val:
                    ...
                    if verbose:
                        print('Epoch {}, best_auc_val: {}'.format(i, best_auc_val))
        ...
        output = model.forward(data.feat_full, adj_full_norm)
        logits_test = output[data.test_mask]

        loss_test = F.nll_loss(logits_test, labels_test)
        auc_test = compute_roc_auc(labels_test, logits_test)
        res.append(auc_test)
        print("Test set results:",
            "loss= {:.4f}".format(loss_test.item()),
            "roc_auc= {:.4f}".format(auc_test))
    ...
    def test_from_saved(self, feat_path=None, adj_path=None, runs_eval=None, verbose=True):
        ...
        res_std = all_res.std()
        print(f"[Saved] Mean ROC AUC: {res_mean:.4f} ± {res_std:.4f}")
        return all_res
    ...
    def train_compression(self):
        ...
        for it in range(self.cfg.epochs+1):
            ...
            for ol in range(outer_loop):                
                ...
                if self.cfg.debug:
                    print("conflict_mask node number is {}".format((conflict_mask==True).sum()))
                ...
                # Compute the Laplacian matrix of the synthetic graph
                L_syn = self.get_laplacian(adj_syn)
                # Compute the Dirichlet energy of the gradient field: tr(G'^T * L * G')
                ...
                if self.cfg.debug:
                    print("self.cfg.lambad*loss_reg is {}".format(self.cfg.lambad*loss_reg.item()))
                    if (conflict_mask == True).sum() > 0:
                        log.info(f"""Epoch {it}, Outer Loop {ol}: 
                            conflict_mask node number is {(conflict_mask==True).sum()}, self.cfg.lambad*loss_reg is {self.cfg.lambad*loss_reg.item()}""")
            ...
            if verbose and (it) % 20 == 0:
                ...
                log.info(f'Mean ROC AUC: {repr([res.mean(), res.std()])}')
                log.info(f'Best mean ROC AUC: {repr([best_res_mean, best_res_std])}')

        
@hydra.main(config_path=CONFIG_DIR, config_name="amazon_review", version_base='1.2')
